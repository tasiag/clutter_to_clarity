...
######################################################################################
#                                                                                    #
# adapted from Jan 2020, https://github.com/fkemeth/lpde                             #
#                                                                                    #
#                                                                                    #
######################################################################################
...
def create_initial_conditions(ic, N, icparam=None):
    ...
    if ic == 'randomabs':
        ...
        y0 = np.random.uniform(-icparam, icparam, N) + 1.0j*np.random.uniform(-icparam, icparam, N)
    return y0
...
def integrate(dynamics='', pars=dict({"lambda": 1.0, "gamma": 1.0, "K": 1.0, "omega": False}),
              N=2, tmin=500, tmax=1000, dt=0.01, T=1000, ic='matthews', icparam=None, Ainit=0,
    ...
    if ic == 'manual':
        ...
        y0 = create_initial_conditions(ic, N, icparam=icparam)
    ...
    print(f"init.shape: {Adict['init'].shape}")
    print(f"tt.shape: {Adict['tt'].shape}")
    print(f"xx.shape: {Adict['xx'].shape}")
    print(f"data.shape: {Adict['data'].shape}")
    print(f"an example data point at t=0:5: {Adict['data'][0:5, 23]}")
    return Adict
